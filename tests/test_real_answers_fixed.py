...
def create_mock_test_data():
    """Neo4j에 실제로 존재하는 개념명으로 테스트 데이터 생성"""
    
    # Neo4j에 실제로 존재하는 개념명 사용 (확인됨)
    answers = [
        MockAnswer("1.3 정수와 유리수", True, 45),     # 정답 - 1학년 수와 연산
        MockAnswer("2.1 문자와 식", False, 120),       # 오답 - 2학년 문자와 식
    ]
...
def test_learning_path_with_real_data():
    """Neo4j 호환 데이터로 학습 경로 생성 테스트"""
    
    print("🔍 Neo4j 호환 Express 진단 테스트 시작")
    print("=" * 60)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("success"):
                ...
                if analysis.get('weakConcepts'):
                    print(f"   취약 개념 목록: {', '.join(analysis.get('weakConcepts', []))}")
        ...
        print("   서버가 실행 중인지 확인해주세요: python -m uvicorn AI.app.main:app --reload")
